...
class Servo:
    ...
    def __init__(self):
        ...
        for index in range(len(servo_config['servo_angle'])):
            self.set_servo_angle(index, servo_config['servo_angle'][index])
            time.sleep(1)
    ...
    def arm_move(self):
        inter_x, inter_y = circle_intersection(0, 0, servo_config['upper_arm_length'], self.axisX, self.axisY, servo_config['fore_arm_length'])
        if inter_x == 0 and inter_y == 0:
            return -1
        servo_config['servo_angle'][3] += servo_config['angle'][0] - cosine_law(inter_x, inter_y, 0, 0, self.axisX, self.axisY)
        servo_config['angle'][0] = cosine_law(inter_x, inter_y, 0, 0, self.axisX, self.axisY)
        ...
        if 0 <= servo_config['servo_angle'][3] <= 180 and 0 <= servo_config['servo_angle'][4] <= 180:
            self.set_servo_angle(3, servo_config['servo_angle'][3])
            self.set_servo_angle(4, servo_config['servo_angle'][4])
        else:
            return -1
        return 0
